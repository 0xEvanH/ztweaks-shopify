...
export async function action({request, context}: Route.ActionArgs) {
  ...
  try {
    ...
    const address: Partial<CustomerAddressInput> = {};
...
function NewAddressForm() {
  const newAddress: CustomerAddressInput & {id: string} = {
    address1: '',
    address2: '',
    city: '',
    company: '',
...
export function AddressForm({
  ...
  const action = useActionData<typeof import('./+types/account.addresses').Route>();
  const error = (action as ActionResponse | undefined)?.error?.[addressId];
